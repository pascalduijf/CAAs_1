...
class Segment:
    def __init__(self, sample, chrom, start, end, num_probes, mean):
        ...
        self.num_probes = int(float(num_probes))  
...
def find_segment_files(tumour):
    result = []
    for path in pathlib.Path('cnv_data').glob('*.seg'):
...
def process_tumours(output_file):
    cohorts = ['MSK-IMPACT_CNA_Segments']
...
def main():
    process_tumours('MSKCM_CNV_Analysis.xlsx')
