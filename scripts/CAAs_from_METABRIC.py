...
Survival = collections.namedtuple('Survival', ['case_id', 'os_status', 'os_months'])
...
class SurvivalFile:
    ...
    def parse(file_name):
        with open(file_name) as handle:
            ...
            for row in reader:
                try:
                    os_months = float(row['Overall_Survival_Months'])
                except ValueError:
                    os_months = math.isnan
                yield Survival(row['Patient_ID'], row['Overall_Survival_Status'], os_months)
...
def header():
    ...
    for column in ['Overall_Survival_Status', 'Overall_Survival_Months']:
        yield column
...
def find_segment_files(tumour):
    result = []
    for path in pathlib.Path('cnv_data_metabric').glob('*.seg.txt'):
...
def find_survival_file(tumour):
    for path in pathlib.Path('survival_data_metabric').glob('*.txt'):
        if path.name.startswith(tumour):
            return str(path)
...
def main():
    process_tumours('METABRIC_CNVs.xlsx')
